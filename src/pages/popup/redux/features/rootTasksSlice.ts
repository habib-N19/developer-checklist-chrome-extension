...
const data = [
...

  {
    ...
    tasks: [
      ...
      },
      {
        ...
        taskDescription: 'Avoid setting images to 100% height. Specify a specific height and width for each image.',
        isComplete: false,
      },
      {
  ...
  {
    title: 'Logo',
    subtitle: 'Ensuring the logo is properly configured enhances user experience and professionalism on the website.',
    tasks: [
      {
        taskTitle: 'Logo Link',
        taskDescription:
...

  {
    ...
    tasks: [
      ...
      {
        taskTitle: 'Safari',
        taskDescription: 'Check all pages using Safari to identify and address any issues specific to this browser.',
        isComplete: false,
      },
      {
        taskTitle: 'Firefox',
        taskDescription: 'Check all pages using Firefox to identify and address any issues specific to this browser.',
        isComplete: false,
      },
    ],
  },
...
const rootTasksSlice = createSlice({
  ...
  initialState: data,
  reducers: {
    addTopic: (state, action: TAddTopicAction) => {
      const { title, subtitle } = action.payload;
      state.push({ title, subtitle, tasks: [] });
    },
    addTask: (state, action: TAddTaskAction) => {
      const { topicIndex, task } = action.payload;
      state[topicIndex].tasks.push({ ...task, isComplete: false });
    },
    toggleComplete: (state, action: PayloadAction<{ topicIndex: number; taskIndex: number }>) => {
      const { topicIndex, taskIndex } = action.payload;
      state[topicIndex].tasks[taskIndex].isComplete = !state[topicIndex].tasks[taskIndex].isComplete;
    },
  },
});
export const { addTopic, addTask, toggleComplete } = rootTasksSlice.actions;
