...
const rootTasksSlice = createSlice({
  ...
  reducers: {
    addTopic: (state, action: TAddTopicAction) => {
      ...
      state.initialStateData.push({ title, subtitle, tasks: [] });
    },
    addTask: (state, action: TAddTaskAction) => {
      const { topicIndex, task } = action.payload;
      state.initialStateData[topicIndex].tasks.push({ ...task, isComplete: false });
    },
    toggleComplete: (state, action: PayloadAction<{ topicTitle: string; taskTitle: string; isComplete: boolean }>) => {
      const { topicTitle, taskTitle, isComplete } = action.payload;

      state.initialStateData = state.initialStateData.map(topic => {
